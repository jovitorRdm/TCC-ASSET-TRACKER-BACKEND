...
export class Assignment {
  ...
  set peopleServed(peopleServed: number) {
    this._peopleServed = peopleServed;
  }
      ...
      .object({
        ...
        paymentMethod: z.enum(
          ...
          ],
          {
            errorMap: () =>
              new AppError(
                `'${this._paymentMethod}' não é um tipo de pagamento válido`
              ),
          }
        ),
        accountType: z.enum(
          [AccountType.EVENTADMINISTRATOR, AccountType.RECEPTIONIST],
          {
            errorMap: () =>
              new AppError(
                `'${this._accountType}' não é um tipo de conta válida`
              ),
          }
        ),
        name: z
        ...
        peopleServed: z
          .number({ required_error: ErrorMessages.MSGE08 })
          .gt(0, "Quantidade de pessoas servidas deve ser maior que 0"),
      })
      .partial({
        id: true,
        status: true,
        accountType: !this._accountRequirement ? true : undefined,
      });
